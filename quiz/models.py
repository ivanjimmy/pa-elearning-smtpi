...
class Quiz(models.Model):
    ...
    def attempted_siswas(self):
        return Siswa.objects.filter(siswaanswer__quiz=self).distinct().count()
...
class Question(models.Model):
    ...
    def total_correct_answers(self):
        return SiswaAnswer.objects.filter(question=self, answer=self.answer).count()

    def total_wrong_answers(self):
        return SiswaAnswer.objects.filter(question=self).exclude(answer=self.answer).count()


class SiswaAnswer(models.Model):
    ...
    def __str__(self):
        return self.siswa.name + ' ' + self.quiz.title + ' ' + self.question.question

    # class Meta:
    #     unique_together = ('siswa', 'quiz', 'question')
